...
export default class MineralPath {
	...
	makeDefaultPath() {
		...
		this.start.getNext(0).getNext(0).getNext(0).setNext(new Node(["5.5-6.0 H", "None", "High SP (5.2); strongly magnetic", "Magnetite"]));
		...
		this.start.getNext(1).getNext(0).getNext(0).setNext(new Node(["5.5-6.0 H", "Dark green to black; Cleavage in", "two directions at nearly 90 degrees", "Augite"]))
			...
			.setNext(new Node(["6.0-6.5 H", "Vitreous; gray to reddish brown or", "black; cleavage in two directions", "at 90 degrees; striations on", "cleavage planes" 
		...
		this.start.getNext(1).getNext(0).getNext(1).setNext(new Node(["9.0 H", "Commonly brown-colored;", "barrel-shaped, hexagonal crystals", "with basal striations"
				,"Corundum"]))
			.setNext(new Node(["7.0-7.5 H", "Vitreous to resinous; dark red,", "reddish- or greenish-brown; brittle;", "planar, nonparallel fractures", "common; dodecahedral crystals"
				,"Garnet"]))
			.setNext(new Node(["7.0-7.5 H", "Vitreous; color varies; elongated,", "transparent, striated, triangular", "prisms common", "Tourmaline"]))
			.setNext(new Node(["7.0 H", "Vitreous; color varies; translucent;", "massive, although hexagonal", "crystals common; conchoidal", "fractures" 
		...
		this.start.getNext(1).getNext(1).getNext(0).setNext(new Node(["3.5-4.0 H", "Resinous; Dark brown to", "yellowish-brown; cleavage in six", "directions, not all faces easily", "observed"
				,"Sphalerite"]))
		...
		this.start.getNext(1).getNext(1).getNext(1).setNext(new Node(["5.0-6.0 H", "Dull, earthy luster; red to", "red-brown color and streak; ", "SP (5.0-5.3)"
		...
		this.start.getNext(2).getNext(0).getNext(0).setNext(new Node(["6.0-7.0 H", "Vitreous; pale yellow to dark green;", "cleavage present but poor;", "granular masses of striated prisms"
				,"Epidote"]))
			.setNext(new Node(["6.0-6.5 H", "Vitreous; white to gray; cleavage", "in two directions at 90°; striations", "(grooves) appear on cleavage", "planes"
				,"Plagioclase (Na-rich feldspar)"]))
			.setNext(new Node(["6.0 H", "Vitreous; white to fleshy-pink;", "cleavage in two directions at 90°;", "muscle fibers (veins) appear on", "cleavage planes"
				,"Orthoclase (K-rich feldspar)"]))
			.setNext(new Node(["4.5-5.0 or 6.5-7.0 H", "Vitreous; typically light blue;", "cleavage good in one direction;", "slim, elongated parallel crystals;", "softer parallel to long axis,", "harder perpendicular to long axis"
				,"Kyanite"]));

		// step 4 branch 2 branch 0 branch 1
		this.start.getNext(2).getNext(0).getNext(1).setNext(new Node(["7.0-7.5 H", "Vitreous; color varies;", "elongated, transparent, striated,", "triangular prisms common"
				,"Tourmaline"]))
			.setNext(new Node(["7.0 H", "Vitreous; color varies;", "translucent; massive, although", "hexagonal crystals common;", "conchoidal fractures"
				,"Quartz (massive or crystalline)"]))
		...
		this.start.getNext(2).getNext(1).getNext(0).setNext(new Node(["2.5 H", "Vitreous; clear to white or light", "gray; cubic cleavage; salty taste", "Halite"]))
			.setNext(new Node(["3.0 H", "Vitreous; clear to white or light", "gray; rhombohedral cleavage;", "reacts with dilute HCl;", "double-refraction in transparent", "varieties"
				,"Calcite"]))
			.setNext(new Node(["3.5-4.0 H", "Vitreous to pearly; clear,", "white, pink, various colors;", "rhombohedral cleavage; occurs in", "coarsely crystalline masses; reacts", "with dilute HCl only in powdered", "form"
				,"Dolomite"]))
			.setNext(new Node(["2.0 H", "Vitreous to pearly; clear, white,", "pink, various colors; cleavage", "good in one direction forming", "thick sheets"
				,"Gypsum (selenite)"]))
		...
		this.start.getNext(2).getNext(1).getNext(1).setNext(new Node(["1.5-2.5 H", "Vitreous to earthy; yellow;", "crystalline or massive", "Sulfur"]))
			.setNext(new Node(["2.0 H", "Earthy; white to translucent; no", "apparent cleavage; massive,", "fine-grained variety, some with", "chicken-wire structure", "(alabaster); fibrous variety", "(satin spar)"
			...
			.setNext(new Node(["3.5-4.0 H", "Dull, earthy luster; white; fine", "crystalline masses with no", "apparent cleavage; reacts with", "dilute HCl only in powdered form"
				,"Dolomite"]));
	}
}
